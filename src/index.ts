...
app.get('/api/stats', async (c) => {
    ...
    // 【重要修正】安全過濾：排除轉帳類型，但保留分類為 NULL 的交易 (避免誤刪無分類的交易)
    const excludeFilter = ` AND c.name NOT IN ('借入/負債', '帳戶間移轉') AND (c.type != 'TRANSFER' OR c.type IS NULL) `
      ...
      <div id="app" class="w-full flex h-full">
        <aside class="w-64 bg-slate-900 text-slate-300 flex-col hidden md:flex shrink-0 shadow-xl z-20">
          <div class="p-6 text-xl font-bold text-white flex items-center gap-3 border-b border-slate-800">
            <div class="w-8 h-8 bg-emerald-500 rounded-lg flex items-center justify-center text-white text-sm"><i class="fa-solid fa-wallet"></i></div>資產管家
          </div>
          <nav class="p-4 space-y-2 flex-1 overflow-y-auto scroller">
             ...
             <div class="pt-4 pb-2 px-4 flex justify-between items-center text-xs font-bold text-slate-500 uppercase">
                <span>銀行帳戶</span><button @click="changeView('accounts')" class="hover:text-white transition"><i class="fa-solid fa-list-ul"></i> 全部</button>
             </div>
             <div class="space-y-1">
                 <button v-for="acc in accounts" :key="acc.id" @click="openDetail(acc)" :class="['w-full text-left px-4 py-2 rounded-lg text-sm transition flex items-center justify-between group', (currentView==='account_detail' && currentAccount?.id===acc.id) ? 'bg-slate-800 text-white' : 'hover:bg-slate-800/50']"><span class="truncate">{{ acc.name }}</span><span class="text-xs bg-slate-800 px-1.5 py-0.5 rounded text-slate-400 group-hover:bg-slate-700">{{ acc.currency }}</span></button>
             </div>
          </nav>
        </aside>
        ...
        <main class="flex-1 flex flex-col h-full overflow-hidden relative">
            <header class="bg-white p-3 flex justify-between items-center md:hidden border-b border-slate-200 z-20 shrink-0">
                <div class="font-bold text-slate-800 flex items-center gap-2"><i class="fa-solid fa-wallet text-emerald-500"></i> 資產管家</div>
            ...
            <div class="flex-1 overflow-y-auto bg-slate-50 p-4 md:p-8 scroller relative">
                <div v-if="currentView === 'notes'" class="absolute inset-0 flex bg-white md:rounded-2xl md:m-6 md:shadow-xl md:border border-slate-200 overflow-hidden">
                    ...
                    <div :class="['flex-1 flex flex-col bg-slate-50 h-full w-full absolute md:relative transition-transform duration-300', (isMobile && !currentNote) ? 'translate-x-full' : 'translate-x-0', 'md:translate-x-0']">
                        <div v-if="!currentNote" class="flex-1 flex flex-col items-center justify-center text-slate-300"><i class="fa-solid fa-book-open text-6xl mb-4"></i><p>選擇一則筆記開始閱讀</p></div>
                            ...
                            <div class="flex-1 overflow-y-auto p-6 md:p-10">
                                <div class="max-w-3xl mx-auto h-full flex flex-col">
                                    ...
                                    <div class="flex gap-2 mb-6">
                                        <button v-for="tag in ['general','diary','todo','idea']" :key="tag" @click="currentNote.tag=tag" :class="['px-3 py-1 rounded-full text-xs font-bold transition border', currentNote.tag===tag ? getTagActiveColor(tag) : 'bg-white text-slate-400 border-slate-200 hover:border-slate-300']">{{ getTagName(tag) }}</button>
                                    </div>
                ...
                <div v-if="currentView === 'dashboard'" class="max-w-6xl mx-auto space-y-6">
                    <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <button v-for="acc in accounts" :key="acc.id" @click="toggleBank(acc.id)" :class="['px-3 py-1 rounded-full text-xs font-bold transition border flex items-center gap-1', selectedBanks.includes(acc.id) ? 'bg-blue-100 text-blue-800 border-blue-200' : 'bg-slate-50 text-slate-400 border-slate-100 decoration-slate-400']"><i v-if="selectedBanks.includes(acc.id)" class="fa-solid fa-check"></i>{{ acc.name }}</button>
                        </div>
                    </div>
                    <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-wrap items-center gap-4">
                        ...
                        <div class="flex items-center gap-2"><input type="date" v-model="dateRange.start" class="bg-slate-50 border border-slate-300 rounded px-2 py-1 text-sm"><span class="text-slate-400">~</span><input type="date" v-model="dateRange.end" class="bg-slate-50 border border-slate-300 rounded px-2 py-1 text-sm"></div>
                        <div class="flex gap-2"><button @click="fetchStats" class="bg-slate-800 text-white px-3 py-1 rounded text-sm font-bold hover:bg-slate-700">查詢</button><button @click="resetDateRange" class="text-slate-500 px-3 py-1 rounded text-sm hover:bg-slate-100">全部</button></div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="bg-slate-800 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden"><div class="text-slate-400 text-sm mb-1">總淨資產 (TWD)</div><div class="text-3xl font-bold">{{ formatCurrency(totalNetWorth) }}</div><i class="fa-solid fa-shield-cat absolute -bottom-4 -right-4 text-8xl opacity-10"></i></div>
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200"><div class="text-slate-500 text-sm mb-1">區間總收入 <span class="text-xs text-slate-300">(已排除轉帳)</span></div><div class="text-3xl font-bold text-emerald-600">+{{ formatCurrency(stats.income) }}</div></div>
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200"><div class="text-slate-500 text-sm mb-1">區間總支出</div><div class="text-3xl font-bold text-rose-600">-{{ formatCurrency(stats.expense) }}</div></div>
                    </div>
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        <div class="flex justify-between items-center mb-4"><h3 class="font-bold text-slate-700">每月收支趨勢</h3><button @click="showDataLabels = !showDataLabels" :class="['text-xs px-2 py-1 rounded border transition', showDataLabels ? 'bg-slate-100 text-slate-600 border-slate-300' : 'text-slate-400 border-slate-100']"><i :class="['fa-solid', showDataLabels ? 'fa-eye' : 'fa-eye-slash']"></i> 顯示數值</button></div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col h-[500px]">
                            <div class="flex justify-between items-center mb-4 shrink-0"><h3 class="font-bold text-slate-700">分類佔比分析</h3><div class="flex bg-slate-100 rounded p-0.5"><button @click="pieType='EXPENSE'" :class="['text-xs px-3 py-1.5 rounded transition font-bold', pieType==='EXPENSE'?'bg-white shadow text-rose-500':'text-slate-400']">支出</button><button @click="pieType='INCOME'" :class="['text-xs px-3 py-1.5 rounded transition font-bold', pieType==='INCOME'?'bg-white shadow text-emerald-500':'text-slate-400']">收入</button></div></div>
                            <div class="flex-1 flex flex-col md:flex-row items-center gap-4 overflow-hidden">
                                <div class="w-full md:w-1/2 h-64 md:h-full relative"><canvas id="pieChart"></canvas><div v-if="!hasPieData" class="absolute inset-0 flex items-center justify-center text-slate-300 text-sm">無數據</div></div>
                                <div class="w-full md:w-1/2 h-full overflow-y-auto scroller pr-2">
                                    <div v-if="hasPieData" class="space-y-2">
                                        <div v-for="(item, idx) in pieChartLegendData" :key="idx" @click="togglePieSegment(idx)" :class="['flex items-center justify-between text-sm p-2 hover:bg-slate-50 rounded-lg transition cursor-pointer select-none', item.hidden ? 'opacity-40 grayscale decoration-slate-400' : '']">
                                            <div class="flex items-center gap-2"><span class="w-3 h-3 rounded-full shrink-0 shadow-sm transition-transform" :class="item.hidden ? 'scale-75' : ''" :style="{backgroundColor: item.color}"></span><span class="text-slate-700 font-bold truncate max-w-[100px]" :class="item.hidden ? 'line-through' : ''">{{ item.name }}</span></div>
                                            <div class="text-right"><div class="font-mono font-bold text-slate-800">{{ item.amount }}</div><div class="text-xs text-slate-400">{{ item.percent }}</div></div>
                                        </div>
                                    </div>
                        ...
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col h-[500px]">
                            <div class="flex justify-between items-center mb-4"><h3 class="font-bold text-slate-700">常出現的關鍵字 (Top 15)</h3><div class="text-xs text-slate-400 bg-slate-100 px-2 py-1 rounded">總金額</div></div>
                            <div class="flex-1 relative overflow-hidden"><canvas id="keywordChart"></canvas><div v-if="stats.keywords.length===0" class="absolute inset-0 flex items-center justify-center text-slate-300 text-sm">無數據</div></div>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="currentView === 'add'" class="max-w-4xl mx-auto flex flex-col gap-6">
                    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden relative">
                        <button @click="showCategoryModal=true" class="absolute top-4 right-4 text-xs bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1.5 rounded-lg font-bold transition flex items-center gap-1"><i class="fa-solid fa-gear"></i> 分類管理</button>
                        <div class="bg-slate-50 p-4 border-b border-slate-100 flex justify-center mt-6 md:mt-0"><div class="grid grid-cols-4 gap-2 bg-slate-200/50 p-1 rounded-xl w-full max-w-lg"><button v-for="t in ['EXPENSE','INCOME','TRANSFER','TRANSFER_IN']" :key="t" @click="form.type=t" :class="['py-2 rounded-lg text-sm font-bold transition flex items-center justify-center gap-1', form.type===t ? typeColor(t) + ' text-white shadow-md' : 'text-slate-500 hover:text-slate-700']">{{ typeName(t) }}</button></div></div>
                        <div class="p-6 grid gap-6">
                            <div class="grid grid-cols-2 gap-4"><div><label class="text-xs font-bold text-slate-400 uppercase mb-1 block">日期</label><input type="date" v-model="form.date" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-blue-500"></div><div><label class="text-xs font-bold text-slate-400 uppercase mb-1 block">帳戶</label><select v-model="form.account_id" @change="fetchRecent" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-blue-500"><option v-for="acc in accounts" :value="acc.id">{{ acc.name }}</option></select></div></div>
                            <div class="relative"><div class="flex justify-between mb-1"><label class="text-xs font-bold text-slate-400 uppercase">金額</label><label class="text-xs flex items-center gap-2"><input type="checkbox" v-model="isDetailMode" class="rounded text-blue-600"> 明細模式</label></div><input type="number" v-model="form.amount_twd" :readonly="isDetailMode" placeholder="0" :class="['w-full rounded-xl px-4 py-4 text-3xl font-mono font-bold outline-none transition', isDetailMode?'bg-slate-100 text-slate-400':'bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500 text-slate-800']"></div>
                            <div v-if="isDetailMode" class="bg-slate-50 rounded-xl border border-slate-200 p-4 space-y-2"><div v-for="(child, idx) in form.children" :key="idx" class="flex gap-2"><select v-model="child.category_id" class="w-1/3 bg-white border border-slate-200 rounded-lg px-2 py-2 text-sm focus:border-blue-500"><option v-for="c in filteredCategories" :value="c.id">{{ c.name }}</option></select><input type="text" v-model="child.note" placeholder="備註" lang="zh-TW" inputmode="text" class="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500"><input type="number" v-model="child.amount_twd" class="w-24 bg-white border border-slate-200 rounded-lg px-2 py-2 text-sm text-right font-mono focus:border-blue-500"><button @click="removeChild(idx)" class="text-slate-400 hover:text-rose-500"><i class="fa-solid fa-xmark"></i></button></div><button @click="addChild" class="text-xs text-blue-600 font-bold hover:underline">+ 新增一列</button></div>
                            <div v-else class="grid grid-cols-2 gap-4"><div><label class="text-xs font-bold text-slate-400 uppercase mb-1 block">分類</label><select v-model="form.category_id" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-blue-500"><option v-for="c in filteredCategories" :value="c.id">{{ c.name }}</option></select></div><div><label class="text-xs font-bold text-slate-400 uppercase mb-1 block">備註</label><input type="text" v-model="form.note" lang="zh-TW" inputmode="text" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-blue-500"></div></div>
                        </div>
                        <div class="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3"><button @click="submit" :disabled="isSubmitting" class="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold shadow hover:bg-slate-800 transition disabled:opacity-50 w-full md:w-auto">確認記帳</button></div>
                    </div>
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                        <div class="p-4 border-b border-slate-100 font-bold text-slate-600 bg-slate-50 flex justify-between items-center"><span><i class="fa-solid fa-clock-rotate-left mr-2"></i> {{ selectedAccountName }} 近期明細</span></div>
                        <div class="overflow-x-auto"><table class="w-full text-left text-sm"><thead class="text-xs text-slate-400 uppercase bg-white border-b border-slate-100"><tr><th class="p-3">日期</th><th class="p-3">分類</th><th class="p-3">備註</th><th class="p-3 text-right">金額</th><th class="p-3 text-center w-24">操作</th></tr></thead><tbody class="divide-y divide-slate-50"><tr v-for="t in recentTransactions" :key="t.id" class="hover:bg-slate-50 group"><td class="p-3 text-slate-500 font-mono">{{ t.date }}</td><td class="p-3 font-bold text-slate-700">{{ t.category_name }}</td><td class="p-3 text-slate-500">{{ t.note }}</td><td :class="['p-3 text-right font-mono font-bold', getAmountClass(t)]">{{ t.type==='INCOME'?'+':'-' }}{{ formatAmount(t) }}</td><td class="p-3 text-center flex justify-center items-center gap-4"><button @click="openEditModal(t)" class="text-slate-300 hover:text-blue-500 opacity-0 group-hover:opacity-100 transition"><i class="fa-solid fa-pen"></i></button><button @click="deleteTransaction(t.id)" class="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition"><i class="fa-solid fa-trash-can"></i></button></td></tr></tbody></table></div>
                    </div>
                </div>

                <div v-if="currentView === 'accounts'" class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"><div v-for="acc in accounts" :key="acc.id" @click="openDetail(acc)" class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm cursor-pointer hover:border-blue-400 hover:shadow-md transition relative group overflow-hidden"><div class="flex justify-between items-start z-10 relative"><div class="font-bold text-lg text-slate-700">{{ acc.name }}</div><span class="text-xs font-bold bg-slate-100 px-2 py-1 rounded text-slate-500">{{ acc.currency }}</span></div><div class="mt-4 z-10 relative"><div class="text-3xl font-mono font-bold text-slate-800">{{ formatCurrency(acc.balance_twd, acc.currency) }}</div><div class="text-xs text-slate-400 mt-1">目前餘額</div></div><i class="fa-solid fa-building-columns absolute -bottom-4 -right-4 text-8xl text-slate-50 group-hover:text-blue-50 transition"></i></div></div>
                <div v-if="currentView === 'account_detail'" class="max-w-5xl mx-auto h-full flex flex-col"><div class="bg-white rounded-2xl shadow-lg border border-slate-200 flex-1 flex flex-col overflow-hidden"><div class="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50"><button @click="currentView='accounts'" class="text-slate-500 hover:text-slate-800 font-bold text-sm"><i class="fa-solid fa-arrow-left mr-1"></i> 返回</button><div class="text-right"><div class="font-bold text-slate-800">{{ currentAccount?.name }}</div><div class="text-xs text-slate-400">當前餘額: {{ formatCurrency(currentAccount?.balance_twd, currentAccount?.currency) }}</div></div></div><div class="flex-1 overflow-y-auto scroller"><table class="w-full text-left"><thead class="bg-slate-50 text-slate-500 text-xs uppercase sticky top-0 z-10 shadow-sm"><tr><th class="p-4 w-10"></th><th class="p-4">日期</th><th class="p-4">說明</th><th class="p-4">備註</th><th class="p-4 text-right">金額</th><th class="p-4 text-right">結餘</th><th class="p-4 text-center w-24">操作</th></tr></thead><tbody class="divide-y divide-slate-50 text-sm"><template v-for="t in detailTransactions" :key="t.id"><tr :class="['hover:bg-slate-50 group cursor-pointer', t.type==='BALANCE'?'bg-slate-100 text-slate-500 font-bold':'']" @click="t.expanded = !t.expanded"><td class="p-4 text-center"><i v-if="t.children?.length" :class="['fa-solid text-xs text-slate-300', t.expanded?'fa-chevron-down':'fa-chevron-right']"></i></td><td class="p-4 font-mono whitespace-nowrap">{{ t.date }}</td><td class="p-4 font-bold text-slate-700 whitespace-nowrap">{{ t.type==='BALANCE'?'初始餘額':t.category_name }}<div class="text-xs text-slate-400 font-normal" v-if="t.children?.length">{{ t.children.length }} 筆明細</div></td><td class="p-4 text-slate-500 max-w-xs truncate">{{ t.note }}</td><td :class="['p-4 text-right font-mono font-bold whitespace-nowrap', getAmountClass(t)]">{{ t.type==='BALANCE'?'':(t.type==='INCOME'?'+':'-') }}{{ t.amount_twd ? formatCurrency(t.amount_twd, currentAccount?.currency) : '-' }}</td><td class="p-4 text-right font-mono text-slate-500 whitespace-nowrap">{{ formatCurrency(t.running_balance, currentAccount?.currency) }}</td><td class="p-4 text-center flex justify-center items-center gap-4" @click.stop><div v-if="t.type!=='BALANCE'" class="flex gap-4"><button @click="openEditModal(t)" class="text-slate-300 hover:text-blue-500 opacity-0 group-hover:opacity-100 transition"><i class="fa-solid fa-pen"></i></button><button @click="deleteTransaction(t.id)" class="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition"><i class="fa-solid fa-trash-can"></i></button></div></td></tr><tr v-if="t.expanded && t.children?.length" class="bg-slate-50/80 shadow-inner"><td colspan="7" class="p-2 pl-16 pr-4"><div v-for="child in t.children" :key="child.id" class="flex justify-between text-xs text-slate-500 py-1 border-b border-slate-200 last:border-0"><span>{{ child.category_name }} <span class="text-slate-400 ml-1">{{ child.note }}</span></span><span class="font-mono">{{ formatCurrency(child.amount_twd, currentAccount?.currency) }}</span></div></td></tr></template></tbody></table></div></div></div>
      ...
      <script>
        ...
        createApp({
            setup() {
                ...
                const showEditModal = ref(false), showCategoryModal = ref(false), editingId = ref(null), selectedBanks = ref([])
                const newCategoryName = ref(''), newCategoryType = ref('EXPENSE')
                const categories = ref([]), accounts = ref([]), recentTransactions = ref([]), detailTransactions = ref([]), currentAccount = ref(null), stats = ref({ income: 0, expense: 0, monthly: [], categories: [], keywords: [] })
                const notesList = ref([]), currentNote = ref(null)
                ...
                onMounted(async () => { await fetchData(); fetchStats(); fetchNotes() }) // 確保順序：先載入銀行，再抓統計
