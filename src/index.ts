...
app.get('/api/stats', async (c) => {
    ...
    // C. 分類佔比 (Pie Chart) - 只取前 5 大，其他合併為 Other
                ...
                <div v-if="currentView === 'add'" class="max-w-4xl mx-auto flex flex-col gap-6">
                    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
                        ...
                        <div class="p-6 grid gap-6">
                            <div class="grid grid-cols-2 gap-4">
                                <div><label class="label">日期</label><input type="date" v-model="form.date" class="input"></div>
                                <div><label class="label">帳戶</label><select v-model="form.account_id" @change="fetchRecent" class="input"><option v-for="acc in accounts" :value="acc.id">{{ acc.name }}</option></select></div>
                            </div>

                            <div class="relative">
                                <div class="flex justify-between mb-1"><label class="label">金額</label><label class="text-xs flex items-center gap-2"><input type="checkbox" v-model="isDetailMode" class="rounded text-blue-600"> 明細模式</label></div>
                                <input type="number" v-model="form.amount_twd" :readonly="isDetailMode" placeholder="0" :class="['input text-3xl font-mono font-bold', isDetailMode?'bg-slate-100 text-slate-400':'']">
                            </div>
                            ...
                            <div v-if="isDetailMode" class="bg-slate-50 rounded-xl border border-slate-200 p-4 space-y-2">
                                <div v-for="(child, idx) in form.children" :key="idx" class="flex gap-2">
                                    <select v-model="child.category_id" class="input w-1/3 text-sm py-2"><option v-for="c in filteredCategories" :value="c.id">{{ c.name }}</option></select>
                                    <input type="text" v-model="child.note" placeholder="備註" class="input flex-1 text-sm py-2">
                                    <input type="number" v-model="child.amount_twd" class="input w-24 text-right text-sm py-2 font-mono">
                            ...
                            <div v-else class="grid grid-cols-2 gap-4">
                                <div><label class="label">分類</label><select v-model="form.category_id" class="input"><option v-for="c in filteredCategories" :value="c.id">{{ c.name }}</option></select></div>
                                <div><label class="label">備註</label><input type="text" v-model="form.note" class="input"></div>
                            </div>
                        </div>
                        <div class="p-4 bg-slate-50 border-t border-slate-100 flex justify-end">
                            <button @click="submit" :disabled="isSubmitting" class="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold shadow hover:bg-slate-800 transition disabled:opacity-50">確認記帳</button>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                        <div class="p-4 border-b border-slate-100 font-bold text-slate-600 bg-slate-50 flex justify-between items-center">
                            ...
                            <span class="text-xs bg-slate-200 text-slate-500 px-2 py-1 rounded">最近 20 筆</span>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-sm">
                                <thead class="text-xs text-slate-400 uppercase bg-white border-b border-slate-100">
                                    <tr><th class="p-3">日期</th><th class="p-3">分類</th><th class="p-3">備註</th><th class="p-3 text-right">金額</th><th class="p-3 w-10"></th></tr>
                                </thead>
                                <tbody class="divide-y divide-slate-50">
                                    <tr v-for="t in recentTransactions" :key="t.id" class="hover:bg-slate-50 group">
                                        ...
                                        <td class="p-3 text-center"><button @click="deleteTransaction(t.id)" class="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition"><i class="fa-solid fa-trash-can"></i></button></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="flex-1 overflow-y-auto scroller">
                            <table class="w-full text-left">
                                <thead class="bg-slate-50 text-slate-500 text-xs uppercase sticky top-0 z-10 shadow-sm">
                                    <tr><th class="p-4 w-10"></th><th class="p-4">日期</th><th class="p-4">說明</th><th class="p-4 text-right">金額</th><th class="p-4 text-right">結餘</th><th class="p-4 w-10"></th></tr>
                                </thead>
                                <tbody class="divide-y divide-slate-50 text-sm">
                                    <template v-for="t in detailTransactions" :key="t.id">
                                        <tr class="hover:bg-slate-50 group cursor-pointer" @click="t.expanded = !t.expanded">
                                            ...
                                            <td class="p-4 text-center"><button @click.stop="deleteTransaction(t.id)" class="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition"><i class="fa-solid fa-trash-can"></i></button></td>
                                        </tr>
      ...
      <script>
        ...
        createApp({
            setup() {
                ...
                watch(pieType, () => renderCharts()) // 切換圓餅圖類型時重繪
                ...
                const fetchStats = async () => {
                    ...
                    stats.value.monthly = data.monthly
                    stats.value.categories = data.categories
                ...
                const openDetail = async (acc) => {
                    ...
                    // --- 計算 Running Balance (結餘) ---
                    // 邏輯：從當前帳戶餘額 (currentAccount.balance_twd) 倒推
                    // 第1筆的結餘 = 帳戶當前餘額
                    // 第2筆的結餘 = 第1筆結餘 - 第1筆金額 (若第1筆是收入則減，支出則加)
                    let running = acc.balance_twd
                    detailTransactions.value = data.map((t, index) => {
                ...
                const submit = async () => {
                    ...
                    try {
                        await fetch('/api/transactions', { method: 'POST', body: JSON.stringify({ main: {...form.value}, children: isDetailMode.value ? form.value.children : [] }) })
                        form.value.amount_twd=''; form.value.children=[]; form.value.note=''
                        if(isDetailMode.value) addChild()
                        fetchRecent(); fetchData(); alert('記帳成功')
                ...
                const renderCharts = () => {
                    ...
                    barChartInstance = new Chart(ctxBar, {
                        ...
                        data: {
                            labels,
                            datasets: [
                                { label: '收入', data: incomeData, backgroundColor: '#10b981', borderRadius: 4 },
                                { label: '支出', data: expenseData, backgroundColor: '#f43f5e', borderRadius: 4 }
                            ]
                        },
                    ...
                    pieChartInstance = new Chart(ctxPie, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                data: finalPieData.map(d => d.total),
                                backgroundColor: ['#3b82f6', '#f59e0b', '#10b981', '#f43f5e', '#8b5cf6', '#cbd5e1'],
                                borderWidth: 0
                            }]
                        },
                ...
                return {
                    currentView, isSubmitting, isDetailMode, selectedYear, pieType, hasPieData,
                    ...
                    addChild, removeChild, changeView, openDetail, fetchRecent, submit, deleteTransaction, fetchStats
                }
            }
        }).mount('#app')
      </script>
